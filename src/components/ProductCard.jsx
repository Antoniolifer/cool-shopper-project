...
function ProductCard({product}) {
  ...
  useEffect( () => {
    ...
    if(cartContent.find(item => item.title === product.title) !== undefined){
      setInCart(true);
      return;
    }
    setInCart(false);
  ...
  return (
    // <div className="w-8/10 md:w-2/5 lg:w-1/5 h-128 md:h-auto shadow-md shadow-gray-700/50 bg-white rounded-lg flex flex-col justify-between items-center">
    <div className="w-full shadow-md shadow-gray-700/50 bg-white rounded-lg flex flex-col justify-between items-center">
        ...
        <span className="w-full mx-auto text-center">
          ...
          {!inCart ? 
          <button onClick={addHandler} className="bg-violet-500 hover:shadow-md hover:shadow-violet-400/50 text-white rounded hover:bg-violet-600 hover:cursor-pointer h-8 py-1 px-2 mb-2">add to cart</button>
          :
          <button className="bg-gray-100 text-black rounded h-8 py-1 px-2 mb-2">already in cart</button>
          }
        </span>
    </div>
  )
